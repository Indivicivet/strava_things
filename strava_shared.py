...
def load_activity_json(path: Path, require_cadences: bool = True) -> Optional[Activity]:
    ...
    if (
        ...
        print(f"got an activity missing things, {path.stem=}, {require_cadences=}")
        return None
    return Activity(
...
def load_activity_gpx(path: Path) -> Activity:
    with open(path) as f:
        gpx = gpxpy.parse(f)
    if len(gpx.tracks) > 1:
